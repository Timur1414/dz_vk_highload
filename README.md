...
- open.spotify.com — авторизация, регистрация, управление подписками. Веб-клиент (SPA) и нативные клиенты. Выделен в изолированный домен для безопасности.
...
- apresolve.spotify.com — Access Point Resolver. Специализированный шлюз разрешения точек входа для прямого управляющего мультиплексированного протокола Spotify,
...
- image.spotifycdn.com — статический контент и изображения. Раздается через гео-распределенный CDN с агрессивным заголовком кэширования (Cache-Control: public, max-age=31536000, immutable).
...
Google Cloud Platform [22]. Были выбраны следующие 10 кластеров:
...
- us-west1 (Даллес, Орегон)
- southamerica-east1 (Осаско, Сан-Паулу)
- us-south1 (Даллас, Техас)
- asia-east1 (Тайвань)
...
- IX.br расположен в 25 км от southamerica-east1. RTT составляет 1 мс. Крупнейший IXP Южной Америки.
- Equinix Dallas расположен в 20 км от us-south1. RTT составляет 1 мс. Узел связности юга США и Мексики.
- NWAX / Seattle IX расположен вблизи us-west1. RTT составляет 2–4 мс. Тихоокеанский хаб.
- NIXI / Extreme IX расположен в 15 км от asia-south1. RTT составляет 1–2 мс. Главный хаб индийского субконтинента.
- NSW-IX расположен в 10 км от australia-southeast1. RTT составляет 1 мс. Обслуживание всей Океании.
...
Запросы, связанные с биллингом, регистрацией и загрузкой новых треков выполняются только на кластере в Европе.

| Дата-центр                     | Доля | API Прослушивания (сред./пик) | Поиск (сред./пик) | Рекомендации (сред./пик) | Лайки (сред./пик) | Авторизация (сред./пик) | Стриминг аудио CDN (сред./пик) |                                         
|--------------------------------|------|-------------------------------|-------------------|--------------------------|-------------------|-------------------------|--------------------------------|                                                                                                                                                                                           
| europe-west1 (Бельгия)         | 14%  | 140000 / 280000               | 1120 / 2240       | 1820 / 3640              | 350 / 700         | 70 / 140                | 63 / 126 Гб/с                  |                                                                                                   
| europe-west4 (Нидерланды)      | 14%  | 140000 / 280000               | 1120 / 2240       | 1820 / 3640              | 350 / 700         | 70 / 140                | 63 / 126 Гб/с                  |                                                                                                
| us-east1 (Южная Каролина)      | 8%   | 80000 / 160000                | 640 / 1280        | 1040 / 2080              | 200 / 400         | 40 / 80                 | 36 / 72 Гб/с                   |                                                                                                     
| us-central1 (Айова)            | 5%   | 50000 / 100000                | 400 / 800         | 650 / 1300               | 125 / 250         | 25 / 50                 | 22.5 / 45 Гб/с                 |                                                                                                           
| us-west1 (Орегон)              | 6%   | 60000 / 120000                | 480 / 960         | 780 / 1560               | 150 / 300         | 30 / 60                 | 27 / 54 Гб/с                   |                                                                                                               
| southamerica-east1 (Сан-Паулу) | 12%  | 120000 / 240000               | 960 / 1920        | 1560 / 3120              | 300 / 600         | 60 / 120                | 54 / 108 Гб/с                  |                                                                                            
| us-south1 (Даллас)             | 9%   | 90000 / 180000                | 720 / 1440        | 1170 / 2340              | 225 / 450         | 45 / 90                 | 40.5 / 81 Гб/с                 |                                                                                                          
| asia-south1 (Мумбаи)           | 16%  | 160000 / 320000               | 1280 / 2560       | 2080 / 4160              | 400 / 800         | 80 / 160                | 72 / 144 Гб/с                  |                                                                                                     
| asia-east1 (Тайвань)           | 12%  | 120000 / 240000               | 960 / 1920        | 1560 / 3120              | 300 / 600         | 60 / 120                | 54 / 108 Гб/с                  |                                                                                                      
| australia-southeast1 (Сидней)  | 4%   | 40000 / 80000                 | 320 / 640         | 520 / 1040               | 100 / 200         | 20 / 40                 | 18 / 36 Гб/с                   |                                                                                                    

Расположение 30 CDN узлов:

| Регион            | Количество | Города и IXP                                                                                                                                | Доля трафика |
|-------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| Европа            | 8          | Франкфурт (DE-CIX), Лондон (LINX), Амстердам (AMS-IX), Стокгольм (Netnod), Париж (France-IX), Мадрид (ESPANIX), Варшава (EPIX), Милан (MIX) | 28%          |
| Северная Америка  | 7          | Нью-Йорк (NYIIX), Ашберн (Equinix), Чикаго, Даллас, Лос-Анджелес (Any2), Сан-Хосе, Торонто (TorIX)                                          | 19%          |
| Латинская Америка | 6          | Сан-Паулу (IX.br), Рио-де-Жанейро, Мехико (MEX-IX), Керетаро, Буэнос-Айрес (CABASE), Богота                                                 | 21%          |
| Азия              | 7          | Мумбаи (NIXI), Дели, Токио (JPNAP), Сингапур (SGIX), Сеул (KINX), Гонконг (HKIX), Дубай (UAE-IX)                                            | 28%          |
| Океания и ЮАР     | 2          | Сидней (NSW-IX), Йоханнесбург (NAPAfrica)                                                                                                   | 4%           |

20% топовых треков дают 80% всех прослушиваний, значит до дата-центров доходит около 6% трафика.

## 3.3. Схема DNS
В Spotify архитектура DNS разделена на внешний контур (для клиентов) и внутренний контур (инфраструктурный DNS) [5].

Внешний DNS:
DNS стандартный, так как используется кастомная система балансировки. TTL для API-доменов 60 сек. TTL для статики 24 часа.
...
## 3.5. Регулирование трафика между дата-центрами (Access Point Resolver)
Клиент при старте или после потери связи выполняет GET запрос на apresolve.spotify.com.
Бэкенд apresolve формирует ранжированный список точек входа в формате JSON на основе данных системы мониторинга кластеров (число открытых TCP соединений, health check, пропускная способность),
IP клиента, весов дата-центров.
Как происходит расчёт оптимального дата-центра:
- Из заголовка X-Forwarded-For или расширения ECS извлекается префикс подсети пользователя.
- С помощью in-memory префиксного дерева определяется номер системы провайдера и вычисляется RTT до дата-центра.
...
Score = -------------
          RTT + E
```
где W - вес, который устанавливается человеком; U - загрузка дата-центра (CPU и TCP соединения); RTT - RTT от клиента до дата-центра; E - коэффициент сглаживания.

Если кластер основного региона недоступен, клиент берет следующий хост из полученного списка.
Если дата-центр недоступен:
- Через панель управления вес этого дата-центра снижается до 0.
- Новые клиенты получают от apresolve список только рабочих дата-центров.
- Уже активные клиенты со случайной задержкой получают от apresolve пакет RECONNECT. После этого клиент создаёт соединение к новому дата-центру.

Расчет паузы между повторными попытками выполняется по формуле Exponential Backoff with Full Jitter.
