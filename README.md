...
| Входящий поток       | 416 Мб/с | 830 Мб/с |
...
Загрузка аудио в день = 100000 * 45 МБ = 4.5 ТБ в сутки. Пропускная способность = 4.5 ГБ * 8 / 86400 = 416 Мб/с.
...
| Авторизация (парольная)         | 500     | 1000    |
| Авторизация (обновление токена) | 7000    | 14000   |
...
Access токен у Spotify живёт час => пользователь запрашивает новый токен в среднем 3 раза в день.
...
В 2018 году компания мигрировала на Google Cloud Platform, поэтому выбор расположения кластеров будет зависеть от доступных регионов
Google Cloud Platform [22]. Были выбраны следующие 5 кластеров:
...
CDN сеть лучше не проектировать с нуля, а закупить мощности у нескольких CDN провайдеров:
